...
class TodoItem extends React.Component {
  ...
  render() {
    const { title, description, dueDate, priority, notes, done, todo } = this.state;
    const doneClass = done ? 'strikeout' : '';
    const regular = (
      <tr className={doneClass} id={todo.id}>
        <td>{title}</td>
        <td>{description}</td>
        <td>{dueDate}</td>
        <td>{priority}</td>
        ...
        <td>
          <button className="btn btn-sm btn-warning" onClick={this.clickUpdateBtn}>
            Update
          </button>
          <button className="btn btn-sm btn-danger" onClick={this.props.deleteTodo}>
            Delete
          </button>
        </td>
      </tr>
